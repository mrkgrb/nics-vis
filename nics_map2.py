...
def create_colored_map_with_coords(results, filename, pix_per_angstrom, colors, vmin, vmax, coords, bonds, atom_colors, atom_sizes, iso_cont, nics_r, ticks_num, linew, no_legend, iso_not_zz, dipole_moment, show_dipole, or_atm, spec_atom, separated_fig=False):
    """
    ...
    if separated_fig:
        ...
        if show_dipole and np.linalg.norm(dipole_moment) > 0.5:
            ...
            x1 = -dx/2; y1 = -dy/2
            ax.arrow(x1, y1, dx, dy, head_width=0.2*scaling_factor, head_length=0.3*scaling_factor, fc="#00AA00", ec="#00AA00", linewidth=6*scaling_factor, alpha=0.8, zorder=15)
...
"""    
    # ===== DRAW MAP =====
    plt.figure(figsize=(float(image_width) / float(150), float(image_height) / float(150)))
    im = plt.imshow(grid_z, extent=(x.min(), x.max(), y.min(), y.max()), origin="lower", cmap=cmap, aspect="auto", vmin=0, vmax=1)
    
    # Add contour lines
    if iso_cont:
        plt.contour(grid_x, grid_y, grid_z, levels=ticks_cor, colors="#EEEEEE", linewidths=linew)
    
    # Title/axes/legend only if NOT separating
    if not separated_fig:
        cbar = plt.colorbar(im, label=nics_type)
        cbar.set_ticks(ticks_cor)
        cbar.set_ticklabels(ticks_val)
    ...
    # legend (only when not separated and not disabled)
    if not separated_fig and not no_legend:
        handles, labels = plt.gca().get_legend_handles_labels()
        unique = dict(zip(labels, handles))
        plt.legend(unique.values(), unique.keys(), loc="upper right", fontsize="medium")
    
    # If separated, strip axes, margins, and save tightly
    if separated_fig:
        ax = plt.gca()
        ax.set_axis_off()
        ...
        plt.savefig(map_fn, dpi=150, bbox_inches="tight", pad_inches=0)
    else:
        plt.savefig(map_fn, dpi=150, bbox_inches="tight")
    ...
    # ===== STANDALONE COLORBAR =====
    if separated_fig:
        # create a slim figure for a vertical colorbar
        # width ~ 300 px, height to match map aspect visually (you can tweak)
        cb_fig_w = 300 / 150.0
        cb_fig_h = float(image_height) / 150.0
        fig, ax = plt.subplots(figsize=(cb_fig_w, cb_fig_h))
    
        norm = Normalize(vmin=0, vmax=1)
        cb = ColorbarBase(ax, cmap=cmap, norm=norm, orientation='vertical',
                          ticks=ticks_cor)
        cb.set_label(nics_type)
        cb.ax.set_yticklabels(ticks_val)
    
        # tight layout without extra padding
        plt.subplots_adjust(left=0.6, right=0.9, top=0.98, bottom=0.02)
        fig.savefig(cbar_fn, dpi=150, bbox_inches="tight")
        print(f"Saved: {cbar_fn}\n")
        plt.close(fig)
"""       
